...
Although I use PSequel GUI in my videos in this section, 
I recommend using https://dbeaver.io/ which is open source 
and works on both Mac and Windows machines or https://www.beekeeperstudio.io/.
...
The most common issue you will find 
is this: https://stackoverflow.com/questions/11460823/setting-windows-path-for-postgres-tools. 
...
Finally, for Windows users if you go to the QA forum of the previous video and this lecture,
people have posted tips and tricks in case you are running into issues.
...
and interested in installing PostgreSQL along with a GUI (eg. pgAdmin), 
their website has wonderful instructions, 
and so does their wiki (for example, this link is for Debian and Ubuntu based distros).  
...
The "createdb test " command 
and the "psql 'test' " command are the same (at least for Debian/Ubuntu systems) 
from what I saw. 
...
to be a database administrator. This way we can just type in psql 'test'  
from the command line and enter the database without the need of logging 
in as the 'postgres' user, just like Andrei does in the lecture. 
...
and this is done by right-clicking 'Servers' on the left pane, 
and choosing 'Create' > 'Server'. 

We give our server a name, and in the 'Connection' tab 
we type in 'localhost' as the host, 
just like Andrei shows in the lecture, and press 'Save'. 
...
I kept running into an error that it couldn't be completed 
because certain folders were not writable. 
I thought this would be easily remedied by running sudo 
but unfortunately the most current version of homebrew 
no longer allows the use of sudo commands due to security risks. 

My next thought was to my root user and use the macOS GUI interface 
to change the permissions on this folder 
because I am not sure how to do this on the terminal. 

Regardless of being logged in as 'root,' 
the OS would not let me change the permissions of the folder. 
I also attempted to use sudo and change the permissions in terminal 
and it did not work either. After several days of banging my head 
against the wall try all kinds of things  to find a solution, 
I discovered that since El Capitan, 
macOS introduced System Integrity Protection aka 'SIP' or 'rootless.' 
As it turned out, once I disabled SIP, 
logged back into 'root' and changed my regular accounts 
permissions to Read/Write on the problem directories, 
I was able to go back to my regular account 
and successfully execute $ brew install postgresql . 

Now it works and I'm so happy to be returning to my lessons!

Steps to Resolve: (WARNING! these steps require you 
to mess with very critical stuff on your computer. 
it is best to exercise EXTREME caution when performing this 
and to revert the system back to its secure state at the end)
...
2. Write down the directory path that the error says 
    it is not able to write to. (e.g. usr/local/share/man/man7) 
    NOTE: you'll want to actually write this down on paper or 
    take a picture of the screen on your phone 
    because you will not be able to use copy and paste)
3. Enable your 'root' user account if you have not already done so. 
    (https://support.apple.com/en-us/HT204012)
    NOTE: make sure to make a really good password for this account 
    and write it down somewhere safe. This is a powerful account 
    and there's no way to recover the password.
...
6. In Finder menu bar select GO > 
    GO TO FOLDER... (CMND + SHFT + G) and type in the path from Step 2.
...
10. Go back to your regular account, 
    run $ brew uninstall postgresql , 
    then $ brew update  and $ brew doctor . 
...
12. If everything is up and running properly. 
    It is probably best to at least enable SIP again 
    (instruction in the article linked in Step 4).

(To check that everything is working. 
I recommend running $ brew services start postgresql  then $ createdb 'test' . 
In my case, it was when I originally tried to run createdb 
and got "command not found" that I realized something was wrong.)

Resources for this lecture
psql instalation.docx
Fullscreen
